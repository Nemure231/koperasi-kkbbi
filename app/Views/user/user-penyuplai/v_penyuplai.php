<section class="section-margin">
  <div class="container">
    <div class="row">

    <div class="col-lg-12 mt-5 mb-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb text-white-all" style="background-color:#7676ff;">
            <li class="breadcrumb-item"><a href="<?php echo base_url().'/' ?>">Beranda</a></li>
            <?php if($penyuplai): ?>
              <li class="breadcrumb-item active" aria-current="page">Penyuplai Produk</li>
            <?php else  : ?>
              <li class="breadcrumb-item active" aria-current="page">Data Kosong!</li>
            <?php endif; ?>
          
          </ol>
        </nav>
      </div>


      <?php if($penyuplai): ?>

      <div class="col-lg-12 mt-5">
        <div class="row">

        <?php foreach($penyuplai as $pu):?>
            <div class="col-md-3 col-lg-3 col-sm-12">
              <div class="card card-primary">
                <div class="card-header">
                  <h4></h4>
                  <div class="card-header-action">
                    <a href="<?php echo base_url().'/penyuplai'.'/'. ''.$pu['id'].'' ?>" class="btn btn-primary">
                      Lihat semua
                    </a>
                  </div>
                </div>
                <div class="card-body">
                <h5><?php echo $pu['nama']; ?></h5>
                </div>
              </div>
            </div>
            <?php endforeach;?>
        
      
      </div>  
    
    </div>

    <?php else  : ?>

<div class="col-lg-12">

<div class="empty-state" data-height="400">
  <div class="empty-state-icon bg-danger">
    <i class="ti-help"></i>
  </div>
  <h2>Data Kosong</h2>
  <p class="lead">
    Maaf, data yang anda cari tidak ada. Silakan kembali, dan periksa lain kali, terima kasih.
  </p>
  <a href="<?php echo base_url().'/produk' ?>" class="btn btn-warning mt-4">Kembali</a>
</div>

</div>


<?php endif; ?>




      

    </div>
  </div>

</section>